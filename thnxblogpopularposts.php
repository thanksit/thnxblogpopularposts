...
class thnxblogpopularposts extends Module implements WidgetInterface {
	public $css_files = array(
		array(
			'key' => 'thnxblogpopularposts',
			'src' => 'thnxblogpopularposts.css',
			'priority' => 50,
			'media' => 'all',
			'load_theme' => false,
		),
	...
	public function __construct() {
		$this->name = 'thnxblogpopularposts';
		...
		parent::__construct();
		$this->displayName = $this->l('thnxBlog Popular Posts');
		$this->description = $this->l('Display Popular Blog Posts Module');
	...
	public function install() {
		...
		$languages = Language::getLanguages(false);
           	foreach($languages as $lang){
        		Configuration::updateValue('thnxbpp_title_'.$lang['id_lang'],"Popular Posts");
        		Configuration::updateValue('thnxbpp_subtext_'.$lang['id_lang'],"All Popular Posts From thnxBlog");
           	}
        	Configuration::updateValue('thnxbpp_postcount',4);
			return true;
	}
	...
	public function SettingForm() {
		...
		$languages = Language::getLanguages(false);
           	foreach($languages as $lang){
           		$helper->fields_value['thnxbpp_title'][$lang['id_lang']] = Configuration::get('thnxbpp_title_'.$lang['id_lang']);
           		$helper->fields_value['thnxbpp_subtext'][$lang['id_lang']] = Configuration::get('thnxbpp_subtext_'.$lang['id_lang']);
           	}
           	$helper->fields_value['thnxbpp_postcount'] = Configuration::get('thnxbpp_postcount');
		return $helper;
	}
	...
	public function getContent() {
		...
		if (Tools::isSubmit('save' . $this->name)) {
			$languages = Language::getLanguages(false);
               foreach($languages as $lang){
            		Configuration::updateValue('thnxbpp_title_'.$lang['id_lang'],Tools::getvalue('thnxbpp_title_'.$lang['id_lang']));
            		Configuration::updateValue('thnxbpp_subtext_'.$lang['id_lang'],Tools::getvalue('thnxbpp_subtext_'.$lang['id_lang']));
               }
            	Configuration::updateValue('thnxbpp_postcount',Tools::getvalue('thnxbpp_postcount'));
		}
	...
	public function getWidgetVariables($hookName = null, array $configuration = [])
	{
	    if(Module::isInstalled('thnxblog') && Module::isEnabled('thnxblog')){
		    ...
		    $thnxbpp_title = Configuration::get('thnxbpp_title_'.$id_lang);
		    $thnxbpp_subtext = Configuration::get('thnxbpp_subtext_'.$id_lang);
		    $thnxbpp_postcount = Configuration::get('thnxbpp_postcount');
		    $thnxblogposts = array();
		    	$thnxblogposts = thnxpostsclass::GetPopularPosts($thnxbpp_postcount);
		    return array(
	    		'thnxbpp_title' => $thnxbpp_title,
	    		'thnxbpp_subtext' => $thnxbpp_subtext,
	    		'thnxbpp_postcount' => $thnxbpp_postcount,
	    		'hookName' => $hookName,
